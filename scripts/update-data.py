# Scrape data from sites : Gather top Pokemon pick rates and sets
...
with open(f"../data/site-dumps/reg-{reg.lower()}_smogon_info.txt", "w", encoding="utf-8") as s:
...
vr_lines = open("../data/site-dumps/vr_calendar_2024.txt", "r", encoding="utf-8").readlines()
for line in vr_lines:
    if "https://victoryroadvgc.com/wp-content/uploads/sprites/" in line:
        ...
        for mon_name in mons_list:
            all_pokemon = open("../data/txt-lists/pokemon.txt", "r", encoding="utf-8").readlines()
            for pokemon in all_pokemon:
                if mon_name in pokemon:
                    ...
                    if not mon_update:
                        top_used[mon_name] = 1
print(f"Successfully: Accessed Victory Road calendar events and found usage # of Pokemon in Regulation {reg}")
...
pr.close()
print(f"Successfully: Wrote data to reg-{reg.lower()}-pick-rates.csv")


# Access Smogon -> Find top mons that have sets -> Find sets for top mons -> Write to top-sets.csv
top_mons_w_sets = {"Amoonguss": f"https://www.smogon.com/dex/sv/pokemon/amoonguss/vgc24-regulation-{reg.lower()}/"}
...
s_lines = open("../data/site-dumps/smogon_reg-f_info.txt", "r", encoding="utf-8").readlines()
for line in s_lines:
...
print(f"Successfully: Accessed Smogon Regulation {reg} page and found top used Pokemon with sets")

for mon in top_mons_w_sets:
    with open(f"../data/smogon-dumps/{mon.lower()}.txt", "w", encoding="utf-8") as new_mon:
        new_mon.write(requests.get(top_mons_w_sets[mon]).text)
    new_mon.close()
    new_mon_lines = open(f"../data/smogon-dumps/{mon.lower()}.txt", "r", encoding="utf-8").readlines()
    for line in new_mon_lines:
        if f'"format":"VGC24 Regulation {reg}"' in line:
            split_line = re.split(f'"format":"VGC24 Regulation {reg}"', line)[1]
    ...
    for mon in top_mon_sets:
        ...
        for data in top_mon_sets[mon]:
            if top_mon_sets[mon].index(data) < 12:
                ...
                ts.write(f"{data}\n")
ts.close()
print(f"Successfully: Wrote data to reg-{reg.lower()}-top-sets.csv")
